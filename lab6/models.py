    ...
    def __init__(self, prompt_file):
        ...
        try:
            ...
            self.client = openai.OpenAI(
                ...
                default_headers={
                    "x-folder-id": YA_FOLDER_ID  # ← КЛЮЧЕВОЙ МОМЕНТ
                }
            )
            # Формируем путь к модели с использованием идентификатора каталога из .env
            self.model = f"gpt://{YA_FOLDER_ID}/yandexgpt"
            # self.model = f"gpt://{YA_FOLDER_ID}/aliceai-llm/latest"
...
# # Инициализация SQLAlchemy (совместимо с app.py)
# db = SQLAlchemy()

# # Модель истории чата — оставляем без изменений
# class ChatHistory(db.Model):
...
# # ==== ЗАМЕНА: LLMService для Ollama (локально) ====
# class OllamaService:
#     def __init__(self, model_name="qwen2.5:4b", system_prompt="Вы — полезный ассистент."):
...
#             # Отправляем POST-запрос к локальному Ollama
...
# # Инициализация сервиса с нужной моделью (можно заменить на llama3 и т.д.)
# ollama_service = OllamaService(
#     model_name="gemma3:1b",
#     system_prompt="Вы — интеллектуальный помощник. Отвечайте кратко, по делу и на русском языке."
# )
...
#     """
#     return ollama_service.chat(user_message)